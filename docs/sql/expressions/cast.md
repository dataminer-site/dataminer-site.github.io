...
The standard SQL syntax for explicit casting is `CAST(expr AS TYPENAME)`, where `TYPENAME` is a name (or alias) of one of [DataMiner's data types](../data_types/overview). DataMiner also supports the shorthand `expr::TYPENAME`, which is also present in PostgreSQL.
