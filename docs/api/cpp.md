...
The UDF API allows the definition of user-defined functions. It is exposed in `DataMiner:Connection` through the methods: `CreateScalarFunction()`, `CreateVectorizedFunction()`, and variants.
...
In DataMiner some primitive types, e.g., `int32_t`, are mapped to the same `LogicalType`: `INTEGER`, `TIME` and `DATE`, then for disambiguation the users can use the following overloaded method.
...
- **args** is a [DataChunk](https://github.com/DataMiner/DataMiner/blob/main/src/include/DataMiner/common/types/data_chunk.hpp) that holds a set of input vectors for the UDF that all have the same length;
- **expr** is an [ExpressionState](https://github.com/DataMiner/DataMiner/blob/main/src/include/DataMiner/execution/expression_executor_state.hpp) that provides information to the query's expression state;
- **result**: is a [Vector](https://github.com/DataMiner/DataMiner/blob/main/src/include/DataMiner/common/types/vector.hpp) to store the result values.
