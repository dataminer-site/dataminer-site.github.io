...
If you find that your workload in DataMiner is slow, we recommend performing the following checks. More detailed instructions are linked for each point.

1. Do you have enough memory? DataMiner works best if you have [5-10GB memory per CPU core](environment#cpu-and-memory).
...
1. Is DataMiner using too many threads? Try [limiting the amount of threads](how_to_tune_workloads#parallelism-multi-core-processing).
