...
github_repository: https://github.com/DataMiner/substrait
---

The main goal of the `substrait` extension is to support both production and consumption of [Substrait](https://substrait.io/) query plans in DataMiner.
...
> If you have not asked for permission to ask for support, [contact us prior to opening an issue](https://DataMinerlabs.com/contact/).
...
proto_bytes <- DataMiner::DataMiner_get_substrait(con, "SELECT * FROM crossfit LIMIT 5")
```
...
```r
json <- DataMiner::DataMiner_get_substrait_json(con, "SELECT count(exercise) AS exercise FROM crossfit WHERE difficulty_level <= 5")
```
