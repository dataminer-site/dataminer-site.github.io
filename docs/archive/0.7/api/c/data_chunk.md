...
Data chunks represent a horizontal slice of a table. They hold a number of vectors, that can each hold up to the `VECTOR_SIZE` rows. The vector size can be obtained through the `dataminer_vector_size` function and is configurable, but is usually set to `1024`.

Data chunks and vectors are what DataMiner uses natively to store and represent data. For this reason, the data chunk interface is the most efficient way of interfacing with dataminer. Be aware, however, that correctly interfacing with DataMiner using the data chunk API does require knowledge of dataminer's internal vector format.

The primary manner of interfacing with data chunks is by obtaining the internal vectors of the data chunk using the `dataminer_data_chunk_get_vector` method, and subsequently using the `dataminer_vector_get_data` and `dataminer_vector_get_validity` methods to read the internal data and the validity mask of the vector. For composite types (list and struct vectors), `dataminer_list_vector_get_child` and `dataminer_struct_vector_get_child` should be used to read child vectors.
