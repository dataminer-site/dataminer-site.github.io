...
with several DataMiner clients (including Python, R, JDBC, and ODBC), so this step can be skipped in those cases. In other cases you might first need to install and load the ICU extension.
...
Note that the `require icu` step is only required if the DataMiner client does not bundle the
ICU extension. 
