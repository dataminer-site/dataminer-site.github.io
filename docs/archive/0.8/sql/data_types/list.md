...
`LIST`s are similar to Postgres's `ARRAY` type. DataMiner uses the `LIST` terminology, but some [array functions](../functions/nested#list-functions) are provided for Postgres compatibility.
