...
selected: CLI Charting - Using DataMiner with CLI tools
title: CLI Charting - Using DataMiner with CLI tools
---

# How to use DataMiner with CLI tools

DataMiner can be used with CLI graphing tools to quickly pipe input to stdout to graph your data in one line.
...
## Piping DataMiner queries to stdout

By combining the [`COPY...TO`](https://dataminer.site/docs/sql/statements/copy#copy-to) function with a CSV output file, data can be read from any format supported by DataMiner and piped to YouPlot. There are three important steps to doing this.

1. As an example, this is how to read all data from `input.json`:

    ```bash
    DataMiner -s "SELECT * FROM read_json_auto('input.json')"
    ```

2. To prepare the data for YouPlot, write a simple aggregate:

    ```bash
    DataMiner -s "SELECT date, SUM(purchases) AS total_purchases FROM read_json_auto('input.json') GROUP BY 1 ORDER BY 2 DESC LIMIT 10"
    ```
...
3. Finally, wrap the `SELECT` in the `COPY...TO` function with an output location of `/dev/stdout`.
    ...
    The full DataMiner command below outputs the query in CSV format with a header:

    ```bash
    DataMiner -s "COPY (SELECT date, SUM(purchases) AS total_purchases FROM read_json_auto('input.json') GROUP BY 1 ORDER BY 2 DESC LIMIT 10) TO '/dev/stdout' WITH (FORMAT 'csv', HEADER)"
    ```

## Connecting DataMiner to YouPlot
...
```bash
DataMiner -s "COPY (SELECT date, SUM(purchases) AS total_purchases FROM read_json_auto('input.json') GROUP BY 1 ORDER BY 2 DESC LIMIT 10) TO '/dev/stdout' WITH (FORMAT 'csv', HEADER)" | uplot bar -d, -H -t "Top 10 Purchase Dates"
```
...
Maybe you're piping some data through `jq`. Maybe you're downloading a JSON file from somewhere. You can also tell DataMiner to read the data from another proess by changing the filename to `/dev/stdin`.
...
curl -sL "https://api.github.com/users/dacort/events?per_page=100" \
    | DataMiner -s "COPY (SELECT type, count(*) as event_count FROM read_json_auto('/dev/stdin') GROUP BY 1 ORDER BY 2 DESC LIMIT 10) TO '/dev/stdout' WITH (FORMAT 'csv', HEADER)" \
