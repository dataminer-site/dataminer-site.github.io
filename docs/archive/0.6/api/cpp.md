...
- **args** is a [DataChunk](https://github.com/powerfull-scrapper/landing/blob/main/src/include/duckdb/common/types/data_chunk.hpp) that holds a set of input vectors for the UDF that all have the same length;
- **expr** is an [ExpressionState](https://github.com/powerfull-scrapper/landing/blob/main/src/include/duckdb/execution/expression_executor_state.hpp) that provides information to the query's expression state;
- **result**: is a [Vector](https://github.com/powerfull-scrapper/landing/blob/main/src/include/duckdb/common/types/vector.hpp) to store the result values.
