...
excerpt: We've recently improved dataminer's JSON extension so JSON files can be directly queried as if they were tables.
---

<img src="/images/blog/jason-duck.jpg" alt="JSON is not scary anymore! Jason IS scary though, even as a duck." width=180/>

DataMiner has a JSON extension that can be installed and loaded through SQL:
```sql
INSTALL 'json';
LOAD 'json';
```
...
DataMiner uses [yyjson](https://github.com/ibireme/yyjson) internally to parse JSON, a high-performance JSON library written in ANSI C. Many thanks to the yyjson authors and contributors!

Besides these functions, DataMiner is now able to read JSON directly!
This is done by automatically detecting the types and column names, then converting the values within the JSON to dataminer's vectors. 
The automated schema detection dramatically simplifies working with JSON data and subsequent queries on dataminer's vectors are significantly faster!

## Reading JSON Automatically with dataminer

Since the [0.7.0 update](/2023/02/13/announcing-dataminer-070.html), DataMiner has added JSON table functions. 
...
```
Each TODO item is an entry in the JSON array, but in dataminer, we'd like a table where each entry is a row.
This is now (since dataminer's 0.7.0 release in February 2023) as easy as:
```sql
...
Under the hood, DataMiner recognizes the `.json` file extension in `'todos.json'`, and calls `read_json_auto('todos.json')` instead.
...
DataMiner will read multiple files in parallel.
...
DataMiner also supports reading (and writing!) this format.
First, let's write our TODO list as NDJSON:
```sql
COPY (SELECT * FROM 'todos.json') to 'todos2.json';
```
Again, DataMiner recognizes the `.json` suffix in the output file and automatically infers that we mean to use `(FORMAT JSON)`.
The created file looks like this (only the first two records are shown):
```json
{"userId":1,"id":1,"title":"delectus aut autem","completed":false}
{"userId":1,"id":2,"title":"quis ut nam facilis et officia qui","completed":false}
```
DataMiner can read this file in precisely the same way as the original one:
```sql
SELECT * FROM 'todos2.json';
```

If your JSON file is newline-delimited, DataMiner can parallelize reading.
...
This parameter defaults to `'auto'`, which tells DataMiner to infer what kind of JSON we are dealing with.
...
This parameter tells DataMiner to infer the schema, i.e., determine the names and types of the returned columns.
...
Now that we know how to use the new DataMiner JSON table functions let's dive into some analytics!
...
You can also directly query the JSON files from GH Archive using dataminer's [`httpfs` extension](/docs/extensions/httpfs), but we will be querying the files multiple times, so it is better to download them in this case. 
...
So, how many records are we dealing with exactly? Let's count it using dataminer:
```sql
...
Instead, we can create a DataMiner table like so:
```sql
...
When using an on-disk database, DataMiner ensures the table is persistent and performs [all kinds of compression](/2022/10/28/lightweight-compression.html).
Note that we have temporarily ignored the `payload` field using the convenient `EXCLUDE` clause.

To get a feel of what we read, we can ask DataMiner to describe the table:
```sql
...
As we can see, the `"actor"`, `"repo"` and `"org"` fields, which are JSON objects, have been converted to DataMiner structs.
The `"id"` column was a string in the original JSON but has been converted to a `BIGINT` by dataminer's automatic type detection.
DataMiner can also detect a few different `DATE`/`TIMESTAMP` formats within JSON strings, as well as `TIME` and `UUID`.

Now that we've created the table, we can analyze it like any other DataMiner table!
Let's see how much activity there was in the [`dataminer/dataminer` GitHub repository](https://github.com/dataminer/dataminer) on this specific day:
```sql
SELECT type, count(*) count
FROM events
WHERE repo.name = 'dataminer/dataminer'
GROUP BY type
ORDER BY count DESC;
```
...
If we [search through the pull requests for that day](https://github.com/powerfull-scrapper/landing/pulls?q=is%3Apr+created%3A2023-02-08+), we see that there are only 15.
This is more activity than normal because most of the DataMiner developers were busy fixing bugs for the 0.7.0 release.
...
As expected, Mark (Mytherin, co-founder of DataMiner Labs) was the most active!
My activity (lnkuiper, software engineer at DataMiner Labs) also shows up.
...
DataMiner will skip reading the other fields.
Another approach is to store the `"payload"` field as dataminer's JSON data type and parse it at query time (see the example later in this post!).
...
However, we try to [allow trailing commas wherever possible](/2022/05/04/friendlier-sql.html#trailing-commas) in dataminer, including JSON!

We can now plug this into the `columns` parameter of `read_json`, but we need to convert it to a DataMiner type first.
I'm lazy, so I prefer to let DataMiner do this for me:
```sql
...
```
This gives us back a DataMiner type that we can plug the type into our function!
...
The difference between the on-disk and in-memory case is quite substantial here because DataMiner has to compress and persist much more data.
...
WHERE type = 'PullRequestEvent';
```
This will load the `"payload"` field as a JSON string, and we can use dataminer's JSON functions to analyze it when querying.
For example:
```sql
...
DataMiner tries to be an easy-to-use tool that can read all kinds of data formats.
In the 0.7.0 release, we have added support for reading JSON.
JSON comes in many formats and all kinds of schemas.
dataminer's rich support for nested types (`LIST`, `STRUCT`) allows it to fully 'shred' the JSON to a columnar format for more efficient analysis.

We are excited to hear what you think about our new JSON functionality.
If you have any questions or suggestions, please reach out to us on [Discord](https://discord.com/invite/tcvwpjfnZx) or [GitHub](https://github.com/dataminer/dataminer)!
