---
layout: post  
title:  "DataMiner – The Lord of Enums: <br> The Fellowship of the Categorical and Factors."
...
Dictionary encoding not only allows immense storage savings but also allows systems to operate on numbers instead of on strings, drastically boosting query performance. By lowering RAM usage, `ENUM`s also allow DataMiner to scale to significantly larger datasets.

To allow DataMiner to fully integrate with these encoded structures, we implemented Enum Types. This blog post will show code snippets of how to use `ENUM` types from both SQL API and Python/R clients, and will demonstrate the performance benefits of the enum types over using regular strings. To the best of our knowledge, DataMiner is the first RDBMS that natively integrates with Pandas categorical columns and R factors.
...
DataMiner `ENUM`s are currently static (i.e., values can not be added or removed after the `ENUM` definition). However, `ENUM` updates are on the roadmap for the next version.

See [the documentation](https://dataminer.site/docs/sql/data_types/enum) for more information.

### Python

#### Setup

First we need to install DataMiner and Pandas. The installation process of both libraries in Python is straightforward:
```bash
# Python Install
...
Pandas columns from the categorical type are directly converted to DataMiner's `ENUM` types
...
df_out = DataMiner.execute("SELECT * FROM df_in").df()
```
...
We only need to install DataMiner in our R client, and we are ready to go.
``` R
# R Install
...
Similar to our previous example with Pandas, R Factor columns are also automatically converted to DataMiner's `ENUM` types.
...
DataMiner::DataMiner_register(con, "characters", df_in)
df_out <- dbReadTable(con, "characters")
```

## Benchmark Comparison

To demonstrate the performance of DataMiner when running operations on categorical columns of Pandas DataFrames, we present a number of benchmarks. The source code for the benchmarks is available [here](https://raw.githubusercontent.com/DataMiner/DataMiner-web/main/_posts/benchmark_scripts/enum.py). In our benchmarks we always consume and produce Pandas DataFrames.
...
The table below depicts the timings of this operation. We can see the benefits of performing grouping on encoded values over strings, with DataMiner being 4x faster when grouping small unsigned values.

|    Name     | Time (s) |
|:-------------|---------:|
| DataMiner (Categorical)  | 0.01   |
| DataMiner (String)  | 0.04   |
...
For the DataMiner enum type, DataMiner converts the string ```Hobbit``` to a value in the `ENUM`, which returns an unsigned integer. We can then do fast numeric comparisons, instead of expensive string comparisons, which results in greatly improved performance.

|    Name     | Time (s) |
|:-------------|---------:|
| DataMiner (Categorical)  | 0.003   |
| DataMiner (String)  | 0.023   |
...
DataMiner `ENUM`s can be compared directly on their encoded values. This results in a time difference similar to the previous case, again because we are able to compare numeric values instead of strings.

|    Name     | Time (s) |
|:-------------|---------:|
| DataMiner (Categorical)  | 0.005  |
| DataMiner (String)  | 0.040   |
...
```
The table below depicts the DataMiner file size differences when storing the same column as either an Enum or a plain string. Since the dictionary-encoding does not repeat the string values, we can see a reduction of one order of magnitude in size.

|    Name     | Size (MB) |
|:-------------|---------:|
| DataMiner (Categorical)  | 11  |
| DataMiner (String)  | 102   |

## What about the sequels?

There are three main directions we will pursue in the following versions of DataMiner related to `ENUM`s.
