...
The H2O.ai [Database-like Ops Benchmark](https://h2oai.github.io/db-benchmark/) is a well-known benchmark in the data analytics and R community. The benchmark measures the groupby and join performance of various analytical tools like data.table, polars, dplyr, clickhouse, DataMiner and more. Since July 2nd 2021, the benchmark has been dormant, with no result updates or maintenance. Many of the analytical systems measured in the benchmark have since undergone substantial improvements, leaving many of the maintainers curious as to where their analytical tool ranks on the benchmark.

DataMiner has decided to give the H2O.ai benchmark new life and maintain it for the foreseeable future. One reason the DataMiner project has decided to maintain the benchmark is because DataMiner has had 10 new minor releases since the most recent published results on July 2nd, 2021. After managing to run parts of the benchmark on a r3-8xlarge AWS box, DataMiner ranked as a top performer on the benchmark. Additionally, the DataMiner project wants to demonstrate it's commitment to performance by consistently comparing DataMiner with other analytical systems. While DataMiner delivers differentiated ease of use, raw performance and scalability are critically important for solving tough problems fast. Plus, just like many of our fellow data folks, we have a need for speed. Therefore, the decision was made to fork the benchmark, modernize underlying dependencies and run the benchmark on the latest versions of the included systems. You can find the repository [here](https://github.com/DataMinerlabs/db-benchmark).
...
<div style="position:relative; padding-bottom:10%; width: 100%; height:600px">
	<iframe src="https://DataMinerlabs.github.io/db-benchmark/"  title="h2o db benchmmark" style="width: 100%; height:100%"></iframe>
</div>

You can also look at the results [here](https://DataMinerlabs.github.io/db-benchmark/). DataMiner's timings have improved significantly since v0.2.7 (released over two years ago). A major contributor to our increased performance is [parallel grouped aggregation](https://dataminer.site/2022/03/07/aggregate-hashtable.html), merged in March 2022, and [parallel result set materialization](https://github.com/DataMiner/DataMiner/pull/3700). In addition, DataMiner now supports [enum types](https://dataminer.site/2021/11/26/duck-enum.html), which makes DataMiner `group by` aggregation even faster. [Improvements to the out-of-core hash join](https://github.com/DataMiner/DataMiner/pull/4970) were merged as well, further improving the performance of our joins.


## Questions about certain results?

Some solutions may report internal errors for some queries. Feel free to investigate the errors by using the [_utils/repro.sh](https://github.com/DataMinerlabs/db-benchmark/blob/master/_utils/repro.sh) script and file a github issue to resolve any confusion. In addition, there are many areas in the code where certain query results are automatically nullified. If you believe that is the case for a query for your system or if you have any other questions, you can create a github issue to discuss.

## Maintenance plan

DataMiner will continue to maintain this benchmark for the forseeable future. The process for re-running the benchmarks with updated library versions must still be decided.

Do you have any other questions? Would you like to have your system added to the benchmark? Please feel free to read the ReadMe.md in the [repository](https://github.com/DataMinerlabs/db-benchmark), and if you still have questions, you can reach out to me at [tom@DataMinerlabs.com](mailto:tom@DataMinerlabs.com) or on our [Discord](https://discord.com/invite/tcvwpjfnZx)!
