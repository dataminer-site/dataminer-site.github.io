...
We have [exposed the C++-level relational API to R](https://github.com/dataminer-site-r/blob/main/R/relational.R), so that it is possible to directly construct DataMiner query plans from R. This low-level API is not meant to be used directly, but it is used by duckplyr to transform the dplyr verbs to the DataMiner relational API and thus to query plans. Here is an example:
