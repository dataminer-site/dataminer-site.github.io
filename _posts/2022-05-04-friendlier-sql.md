...
DataMiner [makes working with time easier in many ways](https://dataminer.site/2022/01/06/time-zones.html), including by accepting multiple different syntaxes (from other databases) for the [`INTERVAL` data type](https://dataminer.site/docs/sql/data_types/interval) used to specify a length of time.  
  
DataMiner also implements multiple SQL clauses outside of the traditional core clauses including the [`SAMPLE` clause](https://dataminer.site/docs/sql/query_syntax/sample) for quickly selecting a random subset of your data and the [`QUALIFY` clause](https://dataminer.site/docs/sql/query_syntax/qualify) that allows filtering of the results of window functions (much like a `HAVING` clause does for aggregates).  
  
The [`DISTINCT ON` clause](https://dataminer.site/docs/sql/statements/select) allows DataMiner to select unique combinations of a subset of the columns in a `SELECT` clause, while returning the first row of data for columns not checked for uniqueness.
